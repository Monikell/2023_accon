...
lmer_sla <- lmer(cwm_sla ~ treatment * site + (1|plot) + (1|block),
...
cwm_sla_cld <- 
  as.data.frame(cld(emmeans(lmer_sla, ~treatment*site)))
...
## figure attempt 2 just the raw cwm in a box plot
# data - cwm_sla_summarize



# Assuming your data frame is named cwm_sla_summarize and it has columns you'd like to plot
# Replace 'YourColumn1' and 'YourColumn2' with the actual column names you want to use

# Basic Boxplot
boxplot(cwm_sla_summarize, treatment, cwm_sla,
        main = "SLA CWM by site and treatment",
        xlab = "",
        ylab = "CWM SLA",
        col = "lightblue")
...
# creating info for Temple

colors <- c("Control" = "#E0FFFF", "NPK" = "#7A8B8B")



colnames(cwm_sla_summarize)


## LEMONTREE THIS WORK NEED TO ADD MY OWN SPICE
cwm_sla_summarize

fig_cwm_sla_allsites <- ggplot(cwm_sla_summarize, aes(x = treatment, y = cwm_sla, fill = treatment)) +
  geom_boxplot(color = "black", outlier.color = "black", outlier.shape = NA, outlier.fill = "white") +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 20),
    axis.title.x = element_text(size = 15),
...
fig_cwm_sla_allsites <- ggplot(cwm_sla_summarize, aes(x = treatment, y = cwm_sla, fill = treatment)) +
  ...
  theme(
    ...
    axis.title.y = element_text(size = 15),
    axis.text = element_text(size = 13),
    strip.text = element_text(size = 24),  # Further increase size of facet titles
    panel.border = element_rect(color = "black", fill = NA, size = 2),
    legend.position = "none",  # Remove the legend
...
## PIE USED FIGURE HERE
## attempt 2 larger font and further spaced out boxes
fig_cwm_sla_allsites <- ggplot(cwm_sla_summarize, aes(x = treatment, y = cwm_sla, fill = treatment)) +
  geom_boxplot(color = "black", outlier.color = "black", outlier.shape = NA, outlier.fill = "white") +
  theme_minimal() +
  theme(
  ...
  labs(
    x = "",  # Change this to your desired x-axis label
    y = "CWM SLA (m2/kg)"   # Change this to your desired y-axis label
  ) +
  scale_fill_manual(values = colors) + 
  facet_wrap( ~ site, scale = "free_y")

png('../data/03_rproducts/fig_cwm_sla_allsites.png',
    width = 12, height = 8, units = 'in', res = 1500)
fig_cwm_sla_allsites
dev.off()



## plotting just temple
cwm_sla_temple_only <- cwm_sla_summarize %>%filter(site == "temple")

fig_cwm_sla_temple <- ggplot(cwm_sla_temple_only, aes(x = treatment, y = cwm_sla, fill = treatment)) +
  geom_boxplot(color = "black", outlier.color = "black", outlier.shape = NA, outlier.fill = "white") +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 20),
 ...
 png('../figures/fig_cwm_sla_temple.png',
     width = 12, height = 8, units = 'in', res = 1500)
 fig_cwm_sla_temple
 dev.off()
...
cwm_sla_cld <- as.data.frame(cld(emmeans(lmer_sla, ~treatment, at = list (site = 'temple'))))


# Plot temple making sure values are correct. USE ME
# add border.
ggplot(cwm_sla_cld, 
       aes(x = treatment, y = emmean, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge(), color = "black") +
...
## plotting the raw thing... let see how that goes


# png('../figures/fig_div.temple.png',
#     width = 12, height = 8, units = 'in', res = 1500)
# fig_div.temple
# dev.off()
